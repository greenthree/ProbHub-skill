#!/usr/bin/env node
const fs = require('fs-extra');
const path = require('path');
const os = require('os');
const { execSync } = require('child_process');

// 支持参数 --local，如果带了参数就装在当前目录，否则装在全局
const isLocal = process.argv.includes('--local');
const skillName = 'probhub';

// 确定注入目标路径
const targetDir = isLocal
    ? path.join(process.cwd(), '.claude', 'skills', skillName)
    : path.join(os.homedir(), '.claude', 'skills', skillName);

const sourceDir = path.join(__dirname, '..');

// 安全拦截：防止在源码目录安装（本地安装时适用）
if (isLocal && sourceDir === process.cwd()) {
    console.error('\n[!] 拦截提示：不能在源码目录初始化它本身。');
    process.exit(1);
}

console.log(`\n🚀 正在将 ProbHub 注入到 ${isLocal ? '本地项目' : '全局系统'}的 Claude Skill 库...`);
console.log(`📂 目标路径: ${targetDir}`);

const filesToCopy = ['SKILL.md', 'references', 'scripts'];

try {
    // 确保目标目录的父级结构存在
    fs.ensureDirSync(targetDir);

    // 复制核心文件
    filesToCopy.forEach(item => {
        const srcPath = path.join(sourceDir, item);
        const destPath = path.join(targetDir, item);
        
        if (fs.existsSync(srcPath)) {
            fs.copySync(srcPath, destPath);
            console.log(`  [+] 成功注入: ${item}`);
        }
    });

    console.log('\n📦 检查 Python 运行环境...');
    try {
        execSync('pip install cyaron pypdf', { stdio: 'ignore' });
        console.log('  [+] 依赖安装完成 (cyaron, pypdf)');
    } catch (e) {
        console.log('  [-] 依赖安装跳过，请确认本地已有 cyaron 和 pypdf');
    }

    console.log('\n=======================================');
    console.log('🎉 ProbHub Skill 安装完成！');
    console.log('=======================================');
    console.log('👉 下一步操作：');
    console.log('  1. 启动 Claude Code:  > claude');
    console.log('  2. 你可以直接使用斜杠命令强制调用（无需唤醒词）:');
    console.log('     > /probhub');
    console.log('  3. 或者直接跟它说你想出一道什么题。');
    console.log('=======================================\n');

} catch (err) {
    console.error('初始化失败:', err);
}