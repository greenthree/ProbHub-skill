# ProbHub Legacy Workflow

仅当工作区根目录不存在 `.probhub/workspace.yaml` 时读取并执行本文件。不要在 Workspace Schema v1 工作区中使用 Legacy 的 `meta.json`、`problems.json` 或手工 DOMjudge 配置流程。
...
7. **触发沙箱自检：** Legacy 工作区没有 `.probhub/workspace.yaml`，直接执行 `python scripts/local_judge.py <英文目录名> --jsonl`；不要调用依赖 Schema v1 的 `probhub judge`。沙箱默认复用内容寻址缓存；仅在排查非确定性程序、缓存实现或需要强制完整重跑并刷新缓存时附加 `--no-cache`。
8. **基于沙箱反馈的自我修复闭环：**
...
- **自动编译与 PDF 提取（必须通过脚本执行）：**
  ...
  2. 脚本自动选择提取模式：
     ...
     - **修改旧题**（该题不是最后一题）→ **PDF 文本扫描模式**：编译后由受控 `pypdf` worker 扫描每页文本，搜索”题目 X. {题名}”标题建立页码映射，精确裁剪目标页码范围。
